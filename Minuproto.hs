...
bit64 d = zeros ++ bits
           where bits = showIntAtBase 2 (head.show) (fromIntegral d) ""
                 zeros = ['0' | _ <- [0..63 - length bits]]

bit8 d = zeros ++ bits ++ " "
           where bits = showIntAtBase 2 (head.show) (fromIntegral d) ""
                 zeros = ['0' | _ <- [0..7 - length bits]]
hex2 d = case showHex (fromIntegral d) "" of
          [c] -> '0':[c]
          xx  -> xx

splitBitsLikePtr bits0 =
  let !(d, bits1) = splitAt 16 bits0 in
  let !(c, bits2) = splitAt 16 bits1 in
  let !(b, a)     = splitAt 30 bits2 in
  [a,b,c,d]

showWord bs o =
  let bs' = BS.take 8 (BS.drop (o * 8) bs) in
  concatMap hex2 $ reverse $ BS.unpack bs'

showBits bs o =
  let bs' = BS.take 8 (BS.drop (o * 8) bs) in
  concatMap bit8 $ reverse $ BS.unpack bs'
...
-- trace ("derefStructPointer " ++ show origin ++ "   " ++ show off ++ "  " ++ show (numdata, numptrs)) $
...
parseInterSegmentPointerAt bs segs o =
  ...
  if b == 0
    then --let !_  = trace ("looking up segment " ++ show d ++ " from pointer word " ++ show (bit64 w)) () in
         let bs' = lookupSegment segs (fromIntegral d) in
